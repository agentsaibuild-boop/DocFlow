...
PROFILES: tuple[ProviderProfile, ...] = (
    ProviderProfile(
        ...
        description=(
            ...
            "български фактури. В нашия тест беше с най-добър баланс между "
            "скорост и точност."
        ),
        best_for=[
    ...
    ProviderProfile(
        ...
        description=(
            "Отлично разпознаване на български текст — включително скенирани "
            "документи, дребен шрифт и нестандартни формати. В нашия тест "
            "разпозна доставчика правилно във всеки случай."
        ),
        best_for=[
            "Скенирани фактури",
    ...
    # Mistral OCR profile hidden during the public evaluation phase — see
    # pipeline.PROVIDER_ALIASES comment. The dataclass can be re-added when
    # the model is benchmarked on enough BG invoices to justify exposing it.
)
