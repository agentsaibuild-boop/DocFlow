...
Общ дашборд за всички: кликваш **Логистика**, **Фактури** или **Човешки ресурси**, после влизаш с име и парола за този отдел. **Фактури** е сканирането и извличането на полета; Логистика ползва същия модул; HR има работно място за документи за самоличност.

Качваш PDF или снимка → DocFlow разпознава полетата и артикулите → сваляш `.xlsx`.
...
Точно два модела, всеки за различен сценарий. **Фактури** ползва по подразбиране **Gemini 3.1 Flash Lite** (както на 19 август):
...
# 3. стартирай
# Логистика и Фактури — Streamlit:
streamlit run app.py

# Човешки ресурси — локален екран на този компютър:
pip install "PySide6-Essentials>=6.7"
python "човешки ресурси/старт.py"
```

HR не минава през браузър. Файловете остават локално; за разпознаване се пращат към Gemini.

На http://localhost:8501 кликваш отдела, после влизаш. Първото стартиране създава локален файл `users.json` (не се комитва):

| Име | Парола | Отдел | Модул |
|---|---|---|---|
| `logistika` | `logistika` | Логистика | Фактури (готов) |
| `fakturi` | `fakturi` | Фактури | Сканиране и полета (готов) |
| `hr` | `hr` | Човешки ресурси | Лични карти и документи за самоличност |

Смени паролите преди реална употреба: задай `AUTH_LOGISTICS_PASSWORD`, `AUTH_INVOICES_PASSWORD` и `AUTH_HR_PASSWORD` в `.env`, изтрий `users.json` и стартирай отново.

## Структура на проекта

- `app.py` — общ дашборд → вход по отдел
- `човешки ресурси/старт.py` — локално HR приложение
- `фактури/` — сканиране на фактури (Streamlit)
- `логистика/` — отдел логистика (същият фактурен модул)
- `docflow/auth.py` / `docflow/org.py` — акаунти и отдели
- `docflow/web/` — общ дашборд (Streamlit shell)
